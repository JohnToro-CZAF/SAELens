...
class ActivationsStore:
    ...
    # tokens_column: Literal["tokens", "input_ids", "text", "problem", "solution"]
    ...
    def from_config(
        ...
        if cfg.token_columns is None:
            raise ValueError(
                "You must specify a token column in your configuration."
            )

        return cls(
            model=model,
            dataset=override_dataset or cfg.dataset_path,
            token_columns=cfg.token_columns,
    ...
    def __init__(
        ...
        if token_columns is not None:
            ...
            self.tokens_columns = token_columns
        else:
            # check if it's tokenized
            if "tokens" in dataset_sample.keys():
                self.is_dataset_tokenized = True
                self.tokens_columns = ["tokens"]
            elif "input_ids" in dataset_sample.keys():
                self.is_dataset_tokenized = True
                self.tokens_columns = ["input_ids"]
            elif "text" in dataset_sample.keys():
                self.is_dataset_tokenized = False
                self.tokens_columns = ["text"]
            elif "problem" in dataset_sample.keys():
                self.is_dataset_tokenized = False
                self.tokens_columns = ["problem"]
            elif "solution" in dataset_sample.keys():
                self.is_dataset_tokenized = False
                self.tokens_columns = ["solution"]
            else:
                raise ValueError(
                    """ 
                        If not provide the token_columns,
                        Dataset must have a 'tokens', 'input_ids', 'text', or 'problem' or 'solution' column
                    """
                )
        print(f"Token columns: {self.tokens_columns}")
        
        if self.is_dataset_tokenized:
            ds_context_size = len(dataset_sample[self.tokens_columns[0]])
            ...
            if hasattr(self.dataset, "set_format"):
                self.dataset.set_format(type="torch", columns=self.tokens_columns)  # type: ignore
    ...
    def _iterate_raw_dataset(
        ...
        for row in self.dataset:
            ...
            yield "\n".join([col + ": " + row[col] for col in self.tokens_columns])  # type: ignore
